...
static void avx256DistributeImpl(const uint32_t* bitArray, size_t bitArraySize, Queue** queues) {
  ...
  for (auto i = 0; i < bitArraySize; i+=8) {
    ...
    do {
      // count leading zeros
      __m256i res_vec = afk_mm_lzcnt_epi32(bitfield_vec);

      // build mask, showing non-zero elements
      res_mask = afk_mm256_cmpneq_epi32_mask(bitfield_vec, _mm256_setzero_si256());
      ...
      if (isUsingCompress) {
          ...
          res_vec = afk_mm_mask_compress_epi32(res_vec, res_mask, res_vec);

          //compress offset vector
          res_offset_vec = afk_mm_mask_compress_epi32(res_offset_vec, res_mask, offset_vec);
...
static void avx256DistributeImpl(const uint32_t* bitArray, size_t bitArraySize, Queue** queues) {
  ...
  for (auto i = 0; i < bitArraySize; i += 8) {
    // load 8 entries to vector register
    __m256i bitfield_vec = _mm256_loadu_si256((__m256i*) &bitArray[i]);
...
static void avx512DistributeImpl(const uint32_t* bitArray, size_t bitArraySize, Queue** queues) {
    ...
    for (auto i = 0; i < bitArraySize; i += 8) {
        ...
        do {
            // count leading zeros
            __m512i res_vec = afk_mm_lzcnt_epi32(bitfield_vec);
...
void avx512DistributeCompress(const uint32_t* bitArray, size_t bitArraySize, Queue** queues) {
    ...
    avx512DistributeImpl<false>(bitArray, fast_size, queues);
    basicDistribute<true>(bitArray + fast_size, reminder_size, queues);
}
